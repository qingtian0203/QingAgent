...
2. **矢量 > 字符**：彻底摒弃 Emoji，全站所有图标均须使用高质量的 SVG 线框图（Outline），并且一律搭配 `stroke="currentColor"` 以实现全自动的深浅自适应。
3. **透明叠加 > 实色填充**：背景色不用固定颜色，只用拥有不同透明度梯度的黑/白色填充，配合深浅模式反转色彩逻辑。
4. **状态过渡 > 跳变**：交互动画 0.18s ease，不生硬，不过长
5. **内敛 hover > 高亮 hover**：hover 只轻微提亮（或加深），不做颜色跳变
...
:root {
    ...
    --border:       rgba(255,255,255,0.07); /* 分割线 */
    ...
    --btn-hov:   rgba(255,255,255,0.08);
    --bdg:       rgba(255,255,255,0.06);
...
### ✅ 应该做
- 复用上方 CSS 变量（`var(--btn-bg)` 等），绝不硬编码静态 `rgba(255,255,255)` 作为背景色以免浅色模式下变成“白垫白”导致隐形。
- 按钮统一 `border-radius: 6px~8px`（矩形圆角，不用胶囊形 `border-radius: 20px+`）
- 所有图标采用 SVG Outline，要求：精细线宽 + 一致大小 + `stroke="currentColor" fill="none"`。
- 所有交互加 `transition: all 0.18s ease`
- hover 只做微调（切换为 `--btn-hov`），不做颜色跳变

### ❌ 不应该做
- 实色大面积填充（`background: #667eea`）用于次要按钮
- 胶囊形按钮（`border-radius: 20px+`）用于功能按钮（仅适用于 tag/badge）
- 使用原生的 Emoji 表情（会导致不同平台风格乱飘，且破坏全站单色的克制质感）
- `box-shadow` 用于普通卡片（只在弹窗、急停等极少场合使用）
- 超过 `0.3s` 的 transition
- 新增不支持深/浅两面热更新的独立颜色变量
